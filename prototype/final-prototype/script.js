...
/* Small inline SVG icon set (stroke-based, one consistent stroke
   weight) used in place of emoji throughout the templates below. */
const ICONS = {
  list: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M8.5 6h12M8.5 12h12M8.5 18h12"/><path d="M3.5 6h.01M3.5 12h.01M3.5 18h.01"/></svg>',
  search: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><circle cx="10.5" cy="10.5" r="6.5"/><path d="M20 20l-4.3-4.3"/></svg>',
  checkCircle: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><circle cx="12" cy="12" r="9"/><path d="M7.75 12.5l2.75 2.75L16.5 9"/></svg>',
  check: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M4.5 12.5l5 5L19.5 7"/></svg>'
};
...
function render() {
  ...
  if (window.Enhance) Enhance.afterRender(view);
}
...
function renderHome() {
  return `
    <div class="hero-grid">
      <div class="glass hero-card" data-reveal>
      ...
      <div class="hero-visual" data-reveal style="--reveal-i: 1;">
        <div class="glass mock-card" aria-hidden="true">
          <div class="mock-card-head">
            <span class="mc-title">Tuesday</span>
            <span class="mc-dot-row"><span></span><span></span><span></span></span>
          </div>
          <div class="mock-row">
            <span class="mc-time">09:00</span>
            <span class="mc-name">ICT 111 &middot; 11-A504</span>
            <span class="mc-pill status-badge status-confirmed">Confirmed</span>
          </div>
          <div class="mock-row">
            <span class="mc-time">12:00</span>
            <span class="mc-name">ICT 402 &middot; 6-302B</span>
            <span class="mc-pill status-badge status-confirmed">Confirmed</span>
          </div>
          <div class="mock-row">
            <span class="mc-time">15:00</span>
            <span class="mc-name">ICT 401 &middot; 11-304</span>
            <span class="mc-pill status-badge status-cancelled">Cancelled</span>
          </div>
        </div>
        <div class="mock-badge mock-badge-1" aria-hidden="true"><span class="mb-icon">${ICONS.checkCircle}</span> Room updated</div>
        <div class="mock-badge mock-badge-2" aria-hidden="true"><span class="mb-icon">${ICONS.search}</span> Search any room</div>
      </div>
    </div>

    <div class="bento">
      <div class="card glass" data-reveal>
        <div class="icon" aria-hidden="true">${ICONS.list}</div>
      ...
      <div class="card glass" data-reveal>
        <div class="icon" aria-hidden="true">${ICONS.search}</div>
        <h3>Search by course or room</h3>
        <p>Type a course code or room number to find exactly what you need,
        instantly.</p>
      </div>
      <div class="card glass" data-reveal>
        <div class="icon" aria-hidden="true">${ICONS.checkCircle}</div>
    ...
    <h2>How it works</h2>
    <div class="step-rows glass">
      <div class="step-row" data-reveal>
        <span class="step-num">1</span>
        <div class="step-body">
          <h3>Open your schedule</h3>
          <p>Tap &ldquo;Schedule&rdquo; to see this week's classes grouped by day.</p>
        </div>
      </div>
      <div class="step-row" data-reveal>
    ...
    <p class="admin-hint">
      Are you a lecturer or admin? <a href="#admin-login">Go to the admin area &rarr;</a>
    </p>
  `;
}
...
function renderScheduleShell() {
  return `
    ...
    <div class="day-nav" id="dayNav">
      <button type="button" class="day-chip active" data-day="all" aria-pressed="true">ALL</button>
      <button type="button" class="day-chip" data-day="Monday" aria-pressed="false">MON</button>
      <button type="button" class="day-chip" data-day="Tuesday" aria-pressed="false">TUE</button>
      <button type="button" class="day-chip" data-day="Wednesday" aria-pressed="false">WED</button>
      <button type="button" class="day-chip" data-day="Thursday" aria-pressed="false">THU</button>
    </div>

    <div id="scheduleList"></div>
    <p id="emptyState" class="lead" hidden>No classes match your search or filter.</p>
  `;
}
...
function renderScheduleList() {
  ...
  DAYS.forEach(day => {
    ...
    dayClasses.forEach(cls => {
      ...
      row.style.setProperty("--reveal-i", Math.min(rowIndex, 10));
      rowIndex++;
      row.innerHTML = `
  ...
  document.getElementById("emptyState").hidden = anyShown;
  ...
  return `
    ...
    <form id="reportForm" class="glass form-card" novalidate>
      ...
      <div class="form-group">
        ...
        <div class="hint field-error" id="courseCodeError" role="alert" hidden>Course code is required.</div>
      </div>
      ...
      <div class="form-group">
        ...
        <div class="hint field-error" id="roomNumberError" role="alert" hidden>Room number is required.</div>
      </div>
      ...
      <div class="form-group">
        ...
        <div class="hint field-error" id="issueTypeError" role="alert" hidden>Please select an issue type.</div>
      </div>
...
function renderReportConfirmation(params) {
  ...
  return `
    <div class="glass confirm-wrap">
      <div class="confirm-icon" aria-hidden="true">${ICONS.check}</div>
...
const ADMIN_CREDENTIALS = { email: "admin@ric.ac.th", password: "demopass" };

function renderAdminLogin() {
  return `
    <form id="loginForm" class="glass form-card login-card" novalidate>
      <p class="eyebrow">Admin &amp; Lecturer Area</p>
      <h1>Log In</h1>
      <p class="hint">
        Demo mode: this is a class prototype, not real production security &mdash;
        but the credentials below do need to match exactly, they aren't a free pass-through anymore.
      </p>

      <div class="form-group">
        <label for="email">Email</label>
        <input id="email" type="email" value="admin@ric.ac.th" aria-describedby="loginError">
      </div>

      <div class="form-group">
        <label for="password">Password</label>
        <input id="password" type="password" value="demopass" aria-describedby="loginError">
      </div>

      <div class="hint field-error" id="loginError" role="alert" hidden>Incorrect email or password.</div>
...
function bindAdminLoginEvents() {
  document.getElementById("loginForm").addEventListener("submit", e => {
    ...
    passwordField.setAttribute("aria-invalid", valid ? "false" : "true");
...
function renderAdminDashboard() {
  return `
    ...
    <div class="chip-row" id="reportFilter">
      ...
      <button type="button" class="filter-chip" data-status="Pending" aria-pressed="false">Pending</button>
      <button type="button" class="filter-chip" data-status="In Progress" aria-pressed="false">In Progress</button>
      <button type="button" class="filter-chip" data-status="Resolved" aria-pressed="false">Resolved</button>
      <button type="button" class="filter-chip" data-status="Closed" aria-pressed="false">Closed</button>
    </div>
...
function bindAdminDashboardEvents() {
  ...
  function renderReportsTable() {
    ...
    if (rows.length === 0) {
      reportsBody.innerHTML = `<tr><td colspan="8" class="table-empty">No reports match this filter.</td></tr>`;
      return;
    }
  ...
  document.getElementById("reportFilter").addEventListener("click", e => {
    ...
    if (!chip) return;
    document.querySelectorAll("#reportFilter .filter-chip").forEach(c => {
      c.classList.remove("active");
      c.setAttribute("aria-pressed", "false");
    });
    chip.classList.add("active");
...
function renderAdminEdit(id) {
  ...
  return `
    ...
    <form id="editForm" class="glass form-card">
      ...
      <div class="form-group">
        <fieldset>
          ...
          <div class="radio-option">
            ...
            <label for="statusChanged">Room Changed</label>
          </div>
          <div class="radio-option">
      ...
      <div class="form-group" id="newRoomGroup" ${cls.status === "Room Changed" ? "" : "hidden"}>
        ...
        <div class="hint">Appears because "Room Changed" is selected above &mdash; enter the new room here.</div>
      </div>
      ...
      <div class="btn-row">
        <button type="submit" class="btn btn-primary btn-fill">Save Update</button>
...
function bindAdminEditEvents(id) {
  ...
  // Show/hide the New Room field the moment a status is picked, so there's
  // no ambiguity about whether to fill it in before or after selecting
  // "Room Changed" (IMP-02 in docs/final-improvement-list.md).
  ...
  form.querySelectorAll('input[name="status"]').forEach(radio => {
    radio.addEventListener("change", () => {
      const showRoom = radio.value === "Room Changed" && radio.checked;
      newRoomGroup.hidden = !showRoom;
      if (!showRoom) newRoomInput.value = ""; // avoid submitting a stale room for a non-Room-Changed status
    });
  });
